...
--GENERATE TABLES THAT WILL PROVIDE TOTAL NUMBER OF FLIGHTS
--Group all_delays relation by carrier and date fileds - this will produce granular date for each flight and each day that there was a flight
group_fcount_flNum_date = GROUP filter_canceled BY (UniqueCarrier, FlightNum, FlightDate);

-- Find the number of delays per each day and carrier for each type of the delay
fcount_flNum_date = FOREACH group_fcount_flNum_date GENERATE group, COUNT($1) AS Num_Flights;

--flatten the group containing the Carrier and the date
fcount_flNum_date_flat = FOREACH fcount_flNum_date GENERATE FLATTEN(group) AS (UniqueCarrier, FlightNum, FlightDate), Num_Flights;

--make the composite key for date
final_flNum_fcount_date = FOREACH fcount_flNum_date_flat GENERATE CONCAT(CONCAT(CONCAT(UniqueCarrier,'_'), CONCAT(FlightNum,'_')), FlightDate) AS Key, Num_Flights;
...
--GENERATE TABLES THAT WILL PROVIDE TOTAL NUMBER OF FLIGHTS
--Group all_delays relation by carrier and date fileds - this will produce granular date for each flight and each day that there was a flight
group_fcount_flNum_year = GROUP filter_canceled BY (UniqueCarrier, FlightNum, Year);

-- Find the number of delays per each day and carrier for each type of the delay
fcount_flNum_year = FOREACH group_fcount_flNum_year GENERATE group, COUNT($1) AS Num_Flights;

--flatten the group containing the Carrier and the date
fcount_flNum_year_flat = FOREACH fcount_flNum_year GENERATE FLATTEN(group) AS (UniqueCarrier, FlightNum, Year), Num_Flights;

--make the composite key for date
final_flNum_fcount_year = FOREACH fcount_flNum_year_flat GENERATE CONCAT(CONCAT(CONCAT(UniqueCarrier,'_'), CONCAT(FlightNum,'_')), (chararray)Year) AS Key, Num_Flights;
...
--GENERATE TABLES THAT WILL PROVIDE TOTAL NUMBER OF FLIGHTS
--Group all_delays relation by carrier and date fileds - this will produce granular date for each flight and each day that there was a flight
group_fcount_flNum_month = GROUP filter_canceled BY (UniqueCarrier, FlightNum, Month);

-- Find the number of delays per each day and carrier for each type of the delay
fcount_flNum_month = FOREACH group_fcount_flNum_month GENERATE group, COUNT($1) AS Num_Flights;

--flatten the group containing the Carrier and the date
fcount_flNum_month_flat = FOREACH fcount_flNum_month GENERATE FLATTEN(group) AS (UniqueCarrier, FlightNum, Month), Num_Flights;

--make the composite key for date
final_flNum_fcount_month = FOREACH fcount_flNum_month_flat GENERATE CONCAT(CONCAT(CONCAT(UniqueCarrier,'_'), CONCAT(FlightNum,'_')), (chararray)Month) AS Key, Num_Flights;

-----

----------- PROCESS QUARTERLY DELAYS --------
---------------------------------------
...
----PROCESSING YEAR DATA that also includes Quarter - grouping is done with Quarter in mind --------------
--Group all_delays relation by carrier and year fileds - this will produce granular date for each flight and each day that there was a flight
grouped_carrier_year_quarterly = GROUP filter_canceled BY (UniqueCarrier, FlightNum, Year, Quarter);

-- Find the number of delays per each day and carrier for each type of the delay
fcount_quarterly = FOREACH grouped_carrier_year_quarterly GENERATE group, COUNT($1) AS Num_Flights;
...
STORE final_date INTO 'hbase://delay_date_count'
...
STORE final_year INTO 'hbase://delay_year_count'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'delay:CarrierDelay_Num, delay:WeatherDelay_Num, delay:NASDelay_Num, delay:SecurityDelay_Num, delay:LateAircraftDelay_Num, delay:OtherDelay_Num'
);

STORE final_month INTO 'hbase://delay_month_count'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'delay:CarrierDelay_Num, delay:WeatherDelay_Num, delay:NASDelay_Num, delay:SecurityDelay_Num, delay:LateAircraftDelay_Num, delay:OtherDelay_Num'
);

STORE final_fcount_date INTO 'hbase://date_fcount'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'count:Num_Flights'
);

STORE final_fcount_year INTO 'hbase://year_fcount'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'count:Num_Flights'
);

STORE final_fcount_month INTO 'hbase://month_fcount'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'count:Num_Flights'
);
...
STORE final_flNum_fcount_date INTO 'hbase://flightNum_date_fcount'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'count:Num_Flights'
);

STORE final_flNum_fcount_year INTO 'hbase://flightNum_year_fcount'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'count:Num_Flights'
);

STORE final_flNum_fcount_month INTO 'hbase://flightNum_month_fcount'
USING org.apache.pig.backend.hadoop.hbase.HBaseStorage(
'count:Num_Flights'
);
